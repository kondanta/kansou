...
func TestHandlePostConfig_ValidReplacement(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/config", bytes.NewReader(b))
	...
	if snap.cfg.Dimensions["story"].Weight != 0.70 {
		t.Error("snapshot not updated after POST /api/v1/config")
	}
}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if config.Hash(srv.getSnapshot().cfg) != hashBefore {
				t.Error("snapshot changed after rejected POST /api/v1/config")
			}
		})
	}
}
