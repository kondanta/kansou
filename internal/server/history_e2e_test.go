...
func TestHistoryEndpoints_EndToEnd(t *testing.T) {
	...
	t.Run("list", func(t *testing.T) {
		...
		res := doGet(t, s, "/api/v1/history", &body)
	...
	t.Run("detail", func(t *testing.T) {
		...
		res := doGet(t, s, "/api/v1/history/1", &body)
		if res.StatusCode != http.StatusOK {
			t.Fatalf("status: got %d, want 200", res.StatusCode)
		}
		if len(body) != 1 {
	...
	t.Run("detail invalid id", func(t *testing.T) {
		...
		res := doGet(t, s, "/api/v1/history/not-a-number", &body)
	...
	t.Run("delete", func(t *testing.T) {
		req := doDelete(t, s, fmt.Sprintf("/api/v1/history/%d", scoreIDToDelete))
	...
	t.Run("delete invalid id", func(t *testing.T) {
		req := doDelete(t, s, "/api/v1/history/not-a-number")
...
func TestHistoryEndpoints_DBless(t *testing.T) {
	...
	t.Run("DELETE /history/{score_id}", func(t *testing.T) {
		res := doDelete(t, s, "/api/v1/history/1")
		if res.StatusCode != http.StatusServiceUnavailable {
			t.Fatalf("status: got %d, want 503", res.StatusCode)
		}
	})
